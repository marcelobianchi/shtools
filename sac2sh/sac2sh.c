...
#define SKIPCH 3                /* number of chars skipped at each hdr
                                 * line */
#define MINTRCLIN 4             /* minimum number of lines per trace in
                                 * q-hdr */
...
int loadsac(data * s)
{
    ...
    if (s->h.nvhdr != 6 && s->h.iftype != 1) {
        if (v)
            fprintf(stderr, " (File not real sac file).\n");
        fclose(ent);
        return -1;
    } else {
        if (v)
    ...
    if (i != s->h.npts) {
        if (v)
            fprintf(stderr, " (Can't read data part).\n");
        fclose(ent);
        free(s->y);
        return -1;
    } else {
    ...
    if (s->h.nzyear != -12345 &&
        s->h.nzjday != -12345 &&
        s->h.nzhour != -12345 &&
        s->h.nzmin != -12345 && s->h.nzsec != -12345
        && s->h.nzmsec != -12345) {
        /*
        ...
        if ((ret = mktime(&s->start)) == -1) {
            if (v)
                fprintf(stderr, " (Time reading error).\n");
            fclose(ent);
            free(s->y);
            return -1;
        } else {
        ...
        if ((s->start.tm_yday + 1) != s->h.nzjday) {
            ...
            if (v)
                fprintf(stderr, " (Jtimed) ");
        }

        if (s->h.b != 0.0) {
            if (v)
                fprintf(stderr, " (Applying B)\n");
        ...
        if (v)
            fprintf(stderr, " (No trace start information)\n");
    }
...
int makeshh(data * s)
{
    ...
    if (s->h.evla != -12345.0) {
        ...
        if (v)
            fprintf(stderr, " EVLA:> %s\n", temp);
        copyto(&s->sh, temp);
    }
    if (s->h.evlo != -12345.0) {
        sprintf(temp, "R017:%f~ ", s->h.evlo);
        if (v)
            fprintf(stderr, " EVLO:> %s\n", temp);
        copyto(&s->sh, temp);
    }
    if (s->h.evdp != -12345.0) {
        sprintf(temp, "R014:%f~ ", s->h.evdp);
        if (v)
            fprintf(stderr, " EVDP:> %s\n", temp);
        copyto(&s->sh, temp);
    }
     ...
     */
    /*
     * if (s->h.gcarc != -12345.0){ sprintf(temp,"R011:%f~ ",s->h.gcarc);
     * if (v) fprintf(stderr," GCarc:> %s\n",temp); copyto(&s->sh,temp); } 
     */
     ...
     */
    /*
     * if (s->h.baz != -12345.0){ sprintf(temp,"R012:%f~ ",s->h.baz); if
     * (v) fprintf(stderr," BAZ:> %s\n",temp); copyto(&s->sh,temp); } 
     */
    ...
    if (s->h.nzyear != -12345 &&
        ...
        sprintf(temp, "S021:%2d-%s-%4d_%02d:%02d:%02d.%03d~ ",
                ...
                s->start.tm_hour, s->start.tm_min, s->start.tm_sec,
                s->mstart);
        if (v)
    ...
    if (s->h.a != -12345.0) {
        ...
        sprintf(temp, "S022: %2d-%s-%4d_%02d:%02d:%02d.%03d~ ",
                ...
                Dtemp.tm_year + 1900,
                Dtemp.tm_hour, Dtemp.tm_min, Dtemp.tm_sec, Dtempm);
        if (v)
            fprintf(stderr, " P-Pick:> %s\n", temp);
        copyto(&s->sh, temp);
    }
...
int fillfilename(data * s)
{
    ...
    if (s->infilename == NULL)
        return -1;

    s->outfilenameB = NULL;
    s->outfilenameB =
        malloc(sizeof(char) * (strlen(s->infilename) + 4 + 1));
    if (s->outfilenameB == NULL)
        return -1;
    sprintf(s->outfilenameB, "%s.QBN", s->infilename);
    for (i = 0; i < strlen(s->outfilenameB); i++)
        s->outfilenameB[i] = toupper(s->outfilenameB[i]);

    s->outfilenameH = NULL;
    s->outfilenameH =
        malloc(sizeof(char) * (strlen(s->infilename) + 4 + 1));
    if (s->outfilenameH == NULL)
        return -1;
    sprintf(s->outfilenameH, "%s.QHD", s->infilename);
    for (i = 0; i < strlen(s->outfilenameH); i++)
        s->outfilenameH[i] = toupper(s->outfilenameH[i]);

    if (v)
        fprintf(stderr, " QBN:> %s\n QHD:> %s\n", s->outfilenameB,
                s->outfilenameH);
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    for (ifile = 1; ifile < argc; ifile++) {
         ...
         */
        if (v)
            fprintf(stderr, "Preparing filenames:\n");
        fillfilename(&s);
        ...
        fprintf(stderr, "%s -> (%4d-%2d %8.1fs %8d %6.4f) -> %s %s\n",
                ...
                s.h.npts, s.h.delta, s.outfilenameB, s.outfilenameH);
        writeSH(&s);
